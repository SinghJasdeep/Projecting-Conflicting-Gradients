...
__all__ = ['ModelAgnosticMetaLearning_2']


class ModelAgnosticMetaLearning_2(object):
    ...
    def __init__(self, model, optimizer=None, step_size=0.1, first_order=False,
                 ...
                 loss_function=F.cross_entropy, device=None, PCGrad=False):
    ...
    def get_outer_loss(self, batches):
        mean_outer_losses = []
        mean_outer_loss_total = torch.tensor(0., device=self.device)
        for batch in batches:
            ...
            mean_outer_losses.append(mean_outer_loss)
            mean_outer_loss_total += mean_outer_loss

        return mean_outer_loss_total, results, mean_outer_losses
    ...
    def train_iter(self, dataloader, max_batches=500):
        ...
        while num_batches < max_batches:
            ...
            for batches in zip(dataloader[0],cycle(dataloader[1])):
                ...
                batches = [tensors_to_device(batch, device=self.device) for batch in batches]
                outer_loss, results, outer_losses = self.get_outer_loss(batches)
                yield results
